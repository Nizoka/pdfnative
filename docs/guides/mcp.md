...
> **Tracks the latest published `pdfnative-mcp`** (v1.3.0, built on pdfnative 1.4.0). Full release notes: [pdfnative-mcp releases](https://github.com/Nizoka/pdfnative-mcp/releases). Live package versions — and the `pdfnative` version each one is built on — are shown at the top of the [documentation home](../index.html).
...
`pdfnative-mcp` exposes **17 tools**:
...
| `add_international_text` | 24 scripts (22 non-Latin plus `latin` and `emoji` font codes) with BiDi & OpenType shaping. `lang` accepts `string`, `string[]`, or comma-separated. |
...
| `split_pdf` | Split one PDF into one document per page range — multi-output `{ mode, count, totalBytes, parts[] }` (50 MiB/part, 200 MiB aggregate). |
...
Concatenates **2–50** PDFs into a single document via pdfnative's page-tree API. Encrypted sources are rejected (`ENCRYPTED_SOURCE`); signatures and `/AcroForm` are dropped because page edits invalidate `/ByteRange`; self-contained URI `/Link` annotations are kept.
...
**Errors:** `ENCRYPTED_SOURCE`, `OUTPUT_TOO_LARGE` (50 MiB/part, 200 MiB aggregate), `PDF_PARSE_FAILED`.
...
- **Output size cap** — PDF output is capped at **50 MB** per call. The page-tree tools (`merge_pdfs`, `split_pdf`, `extract_pages`) additionally enforce **50 MiB per part** and a **200 MiB aggregate** ceiling, returning `OUTPUT_TOO_LARGE` when exceeded.
