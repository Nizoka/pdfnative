...
**Errors:** `ENCRYPTED_SOURCE`, `OUTPUT_TOO_LARGE` (50 MiB per emitted PDF; assembly capped by `maxOutputSizeBytes`, default 256 MiB), `PDF_PARSE_FAILED`.
...
| `OUTPUT_TOO_LARGE` | `split_pdf` | An emitted PDF exceeds 50 MiB, or assembly exceeds `maxOutputSizeBytes` (default 256 MiB). |
