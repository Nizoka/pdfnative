...
> **Tracks the latest published `pdfnative-react`** (first implemented release), built on the zero-dependency `pdfnative` engine with **React 19** as a peer dependency. Live package versions — and the `pdfnative` version each one is built on — are shown at the top of the [documentation home](../index.html). Full history: [pdfnative-react releases](https://github.com/Nizoka/pdfnative-react/releases).
