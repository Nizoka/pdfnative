...
- **samples:** `scripts/generators/currency-symbols.ts` (base-14 €£¥¢ +
  embedded ₹₩₪₫₺₽₿ + multi-currency table) verifies the #48 Euro fix end to
  end; `alphabet-telugu` script-coverage sample; the colour-emoji showcase now
...
- **samples:** `scripts/generators/color-emoji-showcase.ts` (two colour-emoji
  PDFs) wired into `npm run test:generate`. `scripts/generators/use-lite-showcase.ts`
