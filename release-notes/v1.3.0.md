...
> _Still zero runtime dependencies. 71 test files / 1982 tests, all green._
...
- **feat(core):** **Opt-in Unicode normalization (`layout.normalize`).**
  ...
  callers. ([src/core/encoding-context.ts](../src/core/encoding-context.ts))
- **fix(crypto):** **CSPRNG-only randomness.** PDF encryption now **throws**
  ...
  ([src/core/pdf-encrypt.ts](../src/core/pdf-encrypt.ts))
