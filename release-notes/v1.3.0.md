...
> _Still zero runtime dependencies. 63 test files / 1908 tests, all green._
...
- **refactor(core):** `buildPDF()` and `buildDocumentPDF()` now delegate to
  ...
  assembly path.
- **test:** 63 test files / 1908 tests, all green. New coverage: colour-emoji
  ...
  per-line MCID uniqueness in wrapped table cells/captions, and the
