...
│   ├── sha.ts            # SHA-1 (VRI/CertID identification only), SHA-384, SHA-512, HMAC-SHA-256
...
tests/            # 2687+ tests (123 files: unit/integration/fuzz/parser/visual/docs) mirroring src/ structure
...
npm run test            # vitest run (2687+ tests, 123 files)
...
- All new code must have tests. 95%+ statements measured at the v1.6.0 release; CI enforces 88/80/85/90. 2687+ tests (123 files)
...
- SVG rendering: `renderSvg()` maps SVG markup to PDF path operators (m, l, c, re, h, S, f); `parseSvgPath()` parses `<path d="...">` data → `SvgSegment[]`
...
- AcroForm appearance streams: generated via `buildAppearanceStreamDict()` — no external viewer dependency
...
- Signature algorithms: RSA PKCS#1 v1.5 (SHA-256/384/512, digest agility v1.7.0) and ECDSA P-256 (SHA-256)
- Crypto module: standalone `src/crypto/` — sha.ts (SHA-1 identification-only, SHA-384/512, HMAC-SHA-256), asn1.ts (DER incl. canonical `derSetOf`), rsa.ts, ecdsa.ts, x509.ts (+SKI/AKI/EKU/AIA/CRL-DP extensions), cms.ts (PKCS#7 + PAdES profiles), cms-utils.ts (unsigned-attribute surgery), rfc3161.ts (timestamps), ocsp.ts, crl.ts, timestamp-provider.ts + revocation-provider.ts (injected transports — the engine never opens a socket)
- `signPdfBytes()`: takes PDF bytes + private key + certificate → signed PDF bytes with embedded CMS. v1.7.0: `profile: 'pades'` (ESS signing-certificate-v2), `digestAlgorithm`, multi-signature `fieldName` selector
- LTV (v1.7.0): `signPdfBytesWithTimestamp()` (RFC 3161 B-T), `collectValidationInfo()`/`embedValidationInfo()`/`addValidationInfo()` (`/DSS` + `/VRI`, B-LT), `addDocumentTimestamp()` (B-LTA), `listSignatures()`; placeholders support `allowMultiple`/`metadata`/`docTimeStamp`. Network only via injected `TimestampProvider`/`RevocationProvider`
...
- PDF tokenizer: `createTokenizer(bytes)` returns a `PdfTokenizer` (type-only interface) scanning tokens one at a time (lazy, streaming-friendly)
- PDF object parser: `parseValue()`, `parseIndirectObject()` + type guards (`isDict`, `isArray`, `isStream`, `isRef`, `isName`) and dict helpers (`dictGet`, `dictGetName`, `dictGetNum`, `dictGetRef`, `dictGetDict`, `dictGetArray`)
- PDF xref parser: `parseXrefTable()` handles both table and stream xref formats, follows `/Prev` chain (`findStartxref`, `getTrailerValue`, `getTrailerRef`)
- PDF reader: `openPdf(bytes, options?)` returns a `PdfReader` object — `pageCount` property, `getPage(n)`, `getInfo()`, `getPageLabels()`, `getAnnotations(n)`, `getPageRef(n)`, `decodeStream()`
- PDF modifier: `createModifier(reader)` returns a `PdfModifier` — `setObject()`, `addRawObject()`, `addAnnotation()`, `save()` with incremental `/Prev` chain (no page add/remove — use `mergePdfs`/`splitPdf`/`extractPages` for page-tree surgery)
- Parser types: `PdfValue`, `PdfName`, `ParsedDict` (root alias of the parser's `PdfDict`), `ParsedArray`, `PdfStream`, `PdfRef` — discriminated union for type-safe parsing
...
- **Test coverage** — 95%+ statements measured at the v1.6.0 release; CI enforces 88/80/85/90 (statements/branches/functions/lines). 2687+ tests (123 files), 89 fuzz tests across 5 files (including recursion/zip-bomb/xref-chain hardening), dual-mode visual-regression suite, performance benchmarks
