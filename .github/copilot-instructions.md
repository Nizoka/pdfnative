...
tests/            # 2309+ tests (100 files: unit/integration/fuzz/parser/visual) mirroring src/ structure
...
npm run test            # vitest run (2309+ tests, 100 files)
npm run test:watch      # vitest (watch mode)
npm run test:coverage   # vitest with v8 coverage (thresholds: 90/80/85/90)
npm run test:generate   # Generate ~219 sample PDFs → test-output/ (incl. extreme/, emoji/, pdfa-latin/, forms/, charts/ baselines)
...
- All new code must have tests. Current: ~95% statement coverage, 2309+ tests (100 files)
...
- **95%+ test coverage** — 2309+ tests (100 files), 48 fuzz edge-cases (including recursion/zip-bomb/xref-chain hardening), dual-mode visual-regression suite, performance benchmarks
