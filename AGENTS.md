...
Quality bar: GAFAM-grade. 2640+ tests, 95%+ statement coverage measured at v1.6.0 (CI enforces ≥88%), blocking veraPDF validation in CI, SLSA provenance on npm.
...
npm run lint               # eslint
npm run test:generate      # produce ~238 sample PDFs → test-output/
