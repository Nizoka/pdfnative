...
- [x] **pdfnative-mcp** — Model Context Protocol server bridging pdfnative to AI clients (Claude Desktop, Cursor, Continue, Zed, ChatGPT): **14 production tools** (`generate_basic_pdf`, `add_table`, `add_barcode`, `add_international_text`, `add_form`, `embed_image`, `prepare_signature_placeholder`, `sign_pdf`, `inspect_pdf`, `verify_pdf`, `add_attachment`, `extract_text`, `validate_pdf`, `extract_attachments`), stdio/HTTP transport, sandboxed file output, token-frugal read modes (`verbosity`/`fields`), and watermark + Unicode normalization options on authoring tools. Built on pdfnative 1.3.0 as of v1.2.0. See [pdfnative-mcp on GitHub](https://github.com/Nizoka/pdfnative-mcp)
